...
class AnalyticsApiService:
    ...
    def get_dashboard_metrics(self) -> dict:
        ...
        insights = [
            ...
            f"Pagos aprobados: {int(overview.get('paid_reservations', 0) or 0)}; pendientes: {int(overview.get('pending_payments', 0) or 0)}.",
        ]
        recommendations = [
        ...
        return {
            ...
            "paid_revenue": float(overview.get("paid_revenue", 0.0) or 0.0),
            "paid_revenue_today": float(overview.get("paid_revenue_today", 0.0) or 0.0),
            "paid_revenue_week": float(overview.get("paid_revenue_week", 0.0) or 0.0),
            "confirmed_reservations": status_confirmed,
            "pending_reservations": status_pending,
            "paid_reservations": int(overview.get("paid_reservations", 0) or 0),
            "pending_payments": int(overview.get("pending_payments", 0) or 0),
            "failed_payments": int(overview.get("failed_payments", 0) or 0),
            "webhook_failures": int(overview.get("webhook_failures", 0) or 0),
            "active_reservations": int(overview.get("active_reservations", total_reservations) or 0),
            "retry_queue": int(overview.get("retry_queue", 0) or 0),
            "conversion_rate": float(overview.get("conversion_rate", 0.0) or 0.0),
            "most_profitable_court": str(overview.get("most_profitable_court") or "--"),
            "average_ticket": float(overview.get("average_ticket", 0.0) or 0.0),
            "peak_payment_hours": list(overview.get("peak_payment_hours") or []),
            "top_court": top_court,
