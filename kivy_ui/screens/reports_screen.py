...
class ReportsScreen(ServiceScreen):
    ...
    def _fetch_reports_data(self) -> dict:
        ...
        summary_text = (
            ...
            f"Ingresos hoy: {format_currency(metrics.get('revenue_today', 0))}\n"
            f"Ingresos semana: {format_currency(metrics.get('revenue_week', 0))}\n"
            f"Confirmadas: {metrics.get('confirmed_reservations', 0)} | Pendientes: {metrics.get('pending_reservations', 0)}\n"
            f"Ocupacion: {metrics['occupancy_rate']:.1f}%\n"
            f"Cancha lider: {metrics.get('top_court', '--')}\n"
